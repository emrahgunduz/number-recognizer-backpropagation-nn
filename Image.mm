...
Image::Image(NSImage *srcImage, int width, int height,  CGInterpolationQuality interpolation, bool imageIsRotatedBy90degrees) {
  ...
  NSBitmapImageRep *bitmapImageRep = [NSBitmapImageRep imageRepWithData:[srcImage TIFFRepresentation]];
...
void Image::normalise() {
	...
	for(int i=0; i<m_width*m_height; i++) {
    int diff = max - min;
		m_imageData[i]= 255 * (m_imageData[i]-min) / (diff != 0 ? diff : 1);
	}
}
...
// take a source NSImage and convert it to greyscale
ImageWrapper *Image::createImage(NSImage *srcImage, int width, int height, bool imageIsRotatedBy90degrees) {
...
NSImage *Image::toNSImage() {
	...
	uint8_t *result=(uint8_t *) calloc(m_width*m_height*sizeof(uint8_t),4);
  ...
  NSBitmapImageRep *bitmapImageRep = [[NSBitmapImageRep alloc] initWithCGImage:image];
  NSImage *resultNSImage=[[[NSImage alloc] init] autorelease];
	[resultNSImage addRepresentation:bitmapImageRep];
...
int findHeightAtX(Image *img, int x) {	
	...
	bool foundTop = false;
	int topY;
	for(topY = 0; topY < img->getHeight(); topY++) {
		if((*img)[topY][x]==0) {
			foundTop = true;
			break;
		}
