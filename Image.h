...
private:
	...
	Image(NSImage *srcImage, int width, int height, CGInterpolationQuality interpolation, bool imageIsRotatedBy90degrees=false);
	void initYptrs();
public:
	...
	static ImageWrapper *createImage(NSImage *srcImage, int width, int height, bool imageIsRotatedBy90degrees=false);
	// edge detection
	...
	// skeltonize
	void skeletonise();
	// convert back to a NSImage for display
	NSImage *toNSImage();
	~Image() {
		if(m_ownsData)
			free(m_imageData);
