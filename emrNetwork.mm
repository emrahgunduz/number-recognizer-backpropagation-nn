  ...
  for (int i = 0; i < layers.count; i++) {
    ...
    for (int j = 0; j < layer.count; j++) {
      ...
      for (int k = 0; k < [node.connections count]; k++) {
        ...
        connectionWeightDelta[i][j].push_back(conn.weightDelta);
  ...
  for (int i = 0; i < layers.count; i++) {
    ...
    for (int j = 0; j < layer.count; j++) {
      ...
      for (int k = 0; k < [node.connections count]; k++) {
        ...
        conn.weightDelta = connectionWeightDelta[i][j][k];
...
- (NSArray*)runWithOutput:(NSArray*)output
{
  @autoreleasepool {
    if(nodeOutput[layerCount-1].size() != output.count) {
      NSLog(@"Output node count and output value count must be same");
      return [NSArray array];
    }
    
    // Set output for the output layer
    for (int i = 0; i< nodeOutput[layerCount-1].size(); i++) {
      nodeOutput[layerCount-1][i] = [[output objectAtIndex:i] doubleValue];
    }
    
    double sum = 0.0f;
    
    // Set hidden and input layer
    for (int i = layerCount-2; i > 0; i--) {
      for (int j = 0; j < nodeOutput[i].size(); j++) {
        ...
        for (int k = 0; k < nodeOutput[i+1].size(); k++) {
          ...
          double b = connectionWeight[i][j][k];
          sum += a * b;
        }
        ...
        if(transferFunctions) {
          nodeOutput[i][j] = [emrTransferFuncs run:[transferFunctions objectAtIndex:i] withDouble:sum isDerivative:YES];
        } else {
          nodeOutput[i][j] = [emrTransferFuncs run:systemTransferFunction withDouble:sum isDerivative:YES];
        }
      }
    }    
  }
  ...
  @autoreleasepool {
    ...
    for (int i = 0; i < nodeOutput[0].size(); i++) {
      @autoreleasepool {
        [input addObject:[NSNumber numberWithDouble:nodeOutput[0][i]]];
      }
    }
  }
...
- (double)trainWithInput:(NSArray *)input desired:(NSArray *)desired trainingRate:(double)trainingRate momentum:(double)momentum
{
  ...
  NSArray *output;
  @autoreleasepool {
    output = [[self runWithInput:input] copy];
...
- (void)addToErrorArray:(double)error
{
